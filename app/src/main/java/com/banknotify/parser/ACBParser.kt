...
class ACBParser : BankParser {
    override val bankCode = "ACB"
    override val bankName = "ACB"
    override val packageNames = listOf("com.acb.acb", "vn.acb", "com.acb.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("ACB", ignoreCase = true)) return null
    ...
    private fun extractAmount(text: String): Double? {
        val p = Regex("""(?:số tiền|so tien|Số tiền)[:\s]*([\d,.]+)\s*VND""", RegexOption.IGNORE_CASE)
        val m = p.find(text)
    ...
    private fun extractSenderName(text: String): String? {
        val p = Regex("""([A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+(?:\s+[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+){1,5})\s*(?:CK|chuyển khoản|chuyển)""", RegexOption.IGNORE_CASE)
        return p.find(text)?.groupValues?.get(1)
    }
