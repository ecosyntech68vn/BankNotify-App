...
class VietinBankParser : BankParser {
    override val bankCode = "CTG"
    override val bankName = "VietinBank"
    override val packageNames = listOf("com.vietinbank", "vn.vietinbank", "com.vietinbank.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("Vietin", ignoreCase = true) && !text.contains("CTG", ignoreCase = true)) return null
    ...
    private fun extractSenderName(text: String): String? {
        val p = Regex("""([A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+(?:\s+[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+){1,5})\s*(?:CK|chuyển)""", RegexOption.IGNORE_CASE)
        return p.find(text)?.groupValues?.get(1)
    }
