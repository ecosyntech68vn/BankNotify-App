...
class HDBankParser : BankParser {
    override val bankCode = "HDB"
    override val bankName = "HDBank"
    override val packageNames = listOf("com.hdbank", "vn.hdbank", "com.hdbank.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("HDBank", ignoreCase = true) && !text.contains("HD Bank", ignoreCase = true)) return null
