...
class VietcombankParser : BankParser {
    ...
    override val packageNames = listOf("com.vietcombank", "com.vietcombank.vcb", "vcb.app.com.vcb")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("VCB", true) && !text.contains("Vietcombank", true)) return null
