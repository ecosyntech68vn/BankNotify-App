...
class TPBankParser : BankParser {
    override val bankCode = "TPB"
    override val bankName = "TPBank"
    override val packageNames = listOf("com.tpbank", "vn.tpbank", "com.tpbank.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("TPBank", ignoreCase = true)) return null
    ...
    private fun extractSenderName(text: String): String? {
        val p = Regex("""([A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+(?:\s+[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+){1,5})\s*(?:chuyển|chuyển khoản)""", RegexOption.IGNORE_CASE)
        return p.find(text)?.groupValues?.get(1)
    }
