...
class VIBParser : BankParser {
    ...
    override val bankName = "VIB"
    override val packageNames = listOf("com.vib", "vn.vib", "com.vib.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("VIB", ignoreCase = true)) return null
    ...
    private fun extractSenderName(text: String): String? {
        val p = Regex("""([A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+(?:\s+[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+){1,5})\s*(?:CMT|chuyển|CK)""", RegexOption.IGNORE_CASE)
        return p.find(text)?.groupValues?.get(1)
    }
