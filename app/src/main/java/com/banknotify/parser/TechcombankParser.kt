...
class TechcombankParser : BankParser {
    ...
    override val packageNames = listOf("com.techcombank", "com.techcombank.techcombankapp", "vn.techcombank")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("Techcombank", true) && !text.contains("TCB", true)) return null
