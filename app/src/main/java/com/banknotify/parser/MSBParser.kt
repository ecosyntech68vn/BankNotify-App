...
class MSBParser : BankParser {
    override val bankCode = "MSB"
    override val bankName = "MSB"
    override val packageNames = listOf("com.msb", "vn.msb", "com.msb.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("MSB", ignoreCase = true)) return null
