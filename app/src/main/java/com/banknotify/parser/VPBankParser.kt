...
class VPBankParser : BankParser {
    override val bankCode = "VPB"
    override val bankName = "VPBank"
    override val packageNames = listOf("com.vpbank", "vn.vpbank", "com.vpbank.app")

    override fun parse(title: String, body: String): Transaction? {
        val text = "$title $body"
        if (!text.contains("VPBank", ignoreCase = true) && !text.contains("VP Bank", ignoreCase = true)) return null
    ...
    private fun extractAmount(text: String): Double? {
        val p = Regex("""(?:nhận|vừa nhận|số tiền)[:\s]*([\d,.]+)\s*VND""", RegexOption.IGNORE_CASE)
    ...
    private fun extractSenderName(text: String): String? {
        val p = Regex("""từ\s+\d{6,20}\s*[-–]\s*([A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+(?:\s+[A-ZÀ-Ỹ][A-ZÀ-Ỹa-zà-ỹ]+)*)""", RegexOption.IGNORE_CASE)
        return p.find(text)?.groupValues?.get(1)
    }

    private fun extractReference(text: String): String? {
        val p = Regex("""(?:GD|giao dịch)[:\s]*([A-Z0-9]{6,20})""", RegexOption.IGNORE_CASE)
        return p.find(text)?.groupValues?.get(1)
    }
